...
class prankCall{
	...
	public function ekse(){ // تم تغييرها لـ public لتعمل من الـ run
	...
	public function loop($many,$sleep=null){ // تم تغييرها لـ public
		$a=0;
		$no = $this->correct($this->number);
		while($a<$many){
			$rand = rand(123456, 999999);
			...
			$ekse = json_decode($x,true);
			if(empty($ekse['challengeID'])){
				echo "Gagal...\r"; // إضافة توضيح للفشل
				continue;
			}else{
				$nn = $a+1;
				echo "[$nn] Sukses\r";
